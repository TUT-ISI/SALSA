...
MODULE mo_ham_salsa_cloud
  ...
  !<--eehol
  REAL(dp), PUBLIC, ALLOCATABLE :: pfrac_salsa(:,:,:)
  REAL(dp), PUBLIC, ALLOCATABLE :: pnact_strat_salsa(:,:,:)
  REAL(dp), PUBLIC, ALLOCATABLE :: prc_strat_salsa(:,:,:)
  REAL(dp), PUBLIC, ALLOCATABLE :: pnact_conv_salsa(:,:,:)
  REAL(dp), PUBLIC, ALLOCATABLE :: prc_conv_salsa(:,:,:)
  !-->eehol

CONTAINS 

  FUNCTION lnS_crit_insol_over_A(pb,pd) RESULT(res)

    ! -----------------------------------------------------------------------
    !
    ! FUNCTION lnS_crit_insol_over_A
    !
    ...
    !! input variables
    REAL(dp), INTENT(in) :: pb, pd


    !! local variables
    REAL(dp), PARAMETER  :: inv3 = 1._dp/3._dp
    REAL(dp)             :: zb_third
    REAL(dp)             :: zgamma_p, zgamma_m
    REAL(dp)             :: zone_thirds, ztwo_thirds
    REAL(dp)             :: res

    zb_third = inv3*pb

    zgamma_p = 2._dp*zb_third**3+pd+SQRT(4._dp*zb_third**3*pd+pd**2)
    zgamma_m = 2._dp*zb_third**3+pd-SQRT(4._dp*zb_third**3*pd+pd**2)
    ...
    zone_thirds = (0.5_dp*zgamma_p)**inv3        +SIGN((0.5_dp*ABS(zgamma_m))**inv3,zgamma_m)
    !zone_thirds = (0.5_dp*zgamma_p)**inv3        +(0.5_dp*zgamma_m)**inv3 !original version
    !raising zgamma_m to the second power first takes care of negative values here
    ztwo_thirds = (((0.5_dp*zgamma_p)**(2._dp))**inv3)+(((0.5_dp*zgamma_m)**(2._dp))**inv3)
    !ztwo_thirds = (0.5_dp*zgamma_p)**(2._dp*inv3)+(0.5_dp*zgamma_m)**(2._dp*inv3) !original version
    !<--eehol

    res = (2._dp*zb_third**2+      zb_third   *zone_thirds+               ztwo_thirds)/&
          (9._dp*zb_third**3+6._dp*zb_third**2*zone_thirds+3._dp*zb_third*ztwo_thirds+pd) 
  END FUNCTION lnS_crit_insol_over_A
  ...
  SUBROUTINE cloud_activation(&
    ...
    !USE mo_echam_cloud_params, ONLY: cthomi ! minimum T for mixed clouds eehol: this is not needed from echam
    ...
    REAL(dp) ::                         &
             ...
             !! --> thk: modified to account for insoluble core             
!             zkoehl,                    & ! argument of exp for critical supersaturation [mol]
             zcc,                       & ! argument one for computation of S_crit
             zdd,                       & ! argument two for computation of S_crit
             !! <-- thk
    ...
    psc(1:kproma,:,:)    =  0._dp
    prc(1:kproma,:,:,:)  = -1._dp  ! this is not calculated here
    ...
    DO jj = ktdia, klev
       DO ii = 1, kproma
          DO jclass = in2a, fn2b
             ! alaak: pvols(ii,jj,jclass,2) = oc + VBS (see code before calling cloud_activation)
    ...
    DO jj = ktdia,klev    ! vertical grid
       DO ii = 1,kproma   ! horizontal grid
          ...
          IF( & ! are the minimal conditions fulfilled?
               ...
               pqm1(ii,jj)>zeps               .AND. &
               ptm1(ii,jj)>(tmelt-35.0_dp)          & !eehol: temp greater than homogenic ice nucleation temperature
               ) THEN  
...
!! <-- thk
             ...
             !-- subranges 1a-2b
             !! tmiinala modified to loop over all subranges with one loop    
             DO jclass = in1a, fn2b
                !! --> thk: modified to account for insoluble core
                !! --> tmiinala: added lactivation flag as a precondition
                IF ( pnaero(ii,jj,jclass) > nlim  .AND. sizeclass(jclass)%lactivation) THEN
                !! <-- thk
                   ...
                   !! --> thk: modified to account for insoluble core
                   zcc = SQRT(3._dp*zbb*zns(jclass)/zaa)
                   zdd = zvols_insol(ii,jj,jclass)*6._dp/(pi*pnaero(ii,jj,jclass))
                   psc(ii,jj,jclass) = EXP(zaa*lnS_crit_insol_over_A(zcc, zdd))-1._dp
                   !! <-- thkSQRT(4._dp*zb_third**3*pd+pd**2)
             ...
             zgamma = argas*ptm1(ii,jj)/(zps*(speclist(id_wat)%moleweight/1000.)) &
                  + (speclist(id_wat)%moleweight/1000.)*zevap**2/(cpd*papm1(ii,jj)*(amd*1.e3)*ptm1(ii,jj))
          ...
          DO jw=1, nw
             ...
             DO jclass = in2a, fn2b
                ...
                !! <-- thk
                   ...
                   znshi = zns(jclass)*sqrt(vhilim(jclass)/vlolim(jclass))
                   
                   !-- lower bound of critical supersaturation
                   !! --> thk: modified to account for insoluble core
                   zcc = SQRT(3._dp*zbb*znshi/zaa)
                   !zdd = (6._dp/pi)*vhilim(jclass)*zvols_insol(ii,jj,jclass)/zvols_tot(ii,jj,jclass)
                   zdd = zvols_insol(ii,jj,jclass)*6._dp/(pi*pnaero(ii,jj,jclass))*sqrt(vhilim(jclass)/vlolim(jclass))
                   
                   sil = EXP(zaa*lnS_crit_insol_over_A(zcc, zdd))-1._dp
                   !! <-- thk
                   ...
                   znslo = zns(jclass)*sqrt(vlolim(jclass)/vhilim(jclass))
                   
                   !-- upper bound of critical supersaturation
                   !! --> thk: modified to account for insoluble core
                   zcc = SQRT(3._dp*zbb*znslo/zaa)
                   !zdd = (6._dp/pi)*vlolim(jclass)*zvols_insol(ii,jj,jclass)/zvols_tot(ii,jj,jclass)
                   zdd = zvols_insol(ii,jj,jclass)*6._dp/(pi*pnaero(ii,jj,jclass))*sqrt(vlolim(jclass)/vhilim(jclass))
                   siu = EXP(zaa*lnS_crit_insol_over_A(zcc, zdd))-1._dp
                   !! <-- thk
             ...
             pfracn(ii,jj,in2a:fn2b) = zfrac(in2a:fn2b)
             pnact( ii,jj,in2a:fn2b) = pfracn(ii,jj,in2a:fn2b)*pnaero(ii,jj,in2a:fn2b)
             pcdncact(ii,jj) = pcdncact(ii,jj) + sum(pnact(ii,jj,in2a:fn2b))*pwpdf(ii,jj,jw)
...
!    USE mo_activ,        ONLY: nw
    ...
    USE mo_activ,       ONLY: nw
#ifdef HAMMOZ
    USE mo_activ,       ONLY: na, swat_max_strat, swat_max_conv
    USE mo_ham_streams, ONLY: frac, nact_strat, rc_strat, nact_conv, rc_conv
#endif
    ...
    REAL(dp), INTENT(in)  :: &
         ...
         pxtm1(kbdim,klev,ntrac)       ! tracer mixing ratios at t-d
    ...
    DO ii = in2b, fn2b
       ...
       zvols(1:kproma,:,ii,5) = prho(1:kproma,:)*pxtm1(1:kproma,:,jt)/speclist(id_du)%density
    END DO
    zvols(1:kproma,:,:,:)=max(0._dp,zvols(1:kproma,:,:,:))
...
!                        prho(1:kproma,:)*pxtm1(1:kproma, :, jt)/speclist(zgroupaq%spid)%density
    ...
    ! << thk
#endif
   !alaak:zvols gets sometimes (really small (~10^-35)) negative values 
   !This makes SQRT() equations sad :(
    zvols(1:kproma,:,:,:)=max(0._dp,zvols(1:kproma,:,:,:))
    ...
    ! computing activation fractions
    ...
    ! diagnostics
    ! NOTE: the below functionality is (for now) just copied from the functions
    ! ham_activ_diag_abdulrazzak_ghan_conv and ham_activ_diag_abdulrazzak_ghan_strat
    ! in mo_ham_activ.f90. However, this will change, as we intend to change the way
    ! that activated fraction and critical radius are stored for SALSA.

    !<--eehol
    IF (.NOT. ALLOCATED(prc_strat_salsa)) ALLOCATE(prc_strat_salsa(kproma,klev,nclass))
    !-->eehol
    ! stratiform
    IF (nactivpdf <= 0) THEN
       DO jw=1, nw
...
#endif
                 prc_strat_salsa(1:kproma,:,krow)=prc(1:kproma,:,jclass,jw)
              ENDWHERE
           END IF
           !>>dod
        END DO
...
#endif
    !<--eehol
    IF (.NOT. ALLOCATED(pfrac_salsa)) ALLOCATE(pfrac_salsa(kproma,klev,nclass))
    IF (.NOT. ALLOCATED(pnact_strat_salsa)) ALLOCATE(pnact_strat_salsa(kproma,klev,nclass))
    IF (.NOT. ALLOCATED(pnact_conv_salsa)) ALLOCATE(pnact_conv_salsa(kproma,klev,nclass))
    IF (.NOT. ALLOCATED(prc_conv_salsa)) ALLOCATE(prc_conv_salsa(kproma,klev,nclass))
    !-->eehol
...
#endif
          !<--eehol
          pnact_strat_salsa(1:kproma,:,jclass)=pnact(1:kproma,:,jclass)
          pfrac_salsa(1:kproma,:,jclass) = pfracn(1:kproma,:,jclass)
          !-->eehol
       END IF
       !>>dod
    END DO
...
#endif
                  prc_conv_salsa(1:kproma,:,krow)=prc(1:kproma,:,jclass,jw)
               ENDWHERE
            END IF
            !>>dod
         END DO
...
#endif
          pnact_conv_salsa(1:kproma,:,krow)=pnact(1:kproma,:,jclass)
       END IF
       !>>dod
    END DO
