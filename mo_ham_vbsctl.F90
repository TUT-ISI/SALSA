...
MODULE mo_ham_vbsctl
  ...
  TYPE, PUBLIC :: t_voc_prec
     ...
     INTEGER :: idt         ! gas phase tracer index (set in mo_init)
  ...
  TYPE, PUBLIC :: t_vbs_group
     ...
     INTEGER :: idt         ! gas phase tracer index (set in mo_init)
     INTEGER, ALLOCATABLE :: &
          idx(:)            ! list of aerosol tracer indices (set in mo_init)
     INTEGER :: id_vols     ! index in vols structure (for SALSA)
     INTEGER :: id_gasspec  ! index for HAM zgas array (set in mo_subm_species)
