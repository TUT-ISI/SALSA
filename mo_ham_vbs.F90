...
CONTAINS
  ...
  SUBROUTINE vbs_species
    ...
    !
    ! vbs_species is called from ham_species in mo_species
    !
    ! -----------------------------------------------------------------------
...
!---Additional test species:
       ...
       CALL new_species(&
            ...
            dryreac     = 0._dp,                        & ! to be reviewed
            henry       = (/ 1.E5_dp, 0._dp /),         & ! to be reviewed
            density     = 1600._dp,                & !From pubchem
!            iaerorad    = iradoc,                      & ! to be reviewed
            lwatsol     = .TRUE.,                       & ! to be reviewed
            kappa       = 0.037_dp,                     & !Petters and Kreidenweis (2007) ! Sampo: To be reviewed
            lburden     = .TRUE.,                       & ! to be reviewed
            idx         = spid_temp                     &
            )


       vbs_set(4)%lcreateaero = .TRUE.
       vbs_set(4)%spid        = spid_temp
       vbs_set(4)%spid_aero   = 0
       vbs_set(4)%C0          = &                    ! equ. vapor con. at T0 [mol/m3] ! Sampo: I'll just use the mass conc since I have it
            2.3746e-08_dp                              ! [speclist%moleweight] = g/mol
       vbs_set(4)%T0          = 298_dp               ! T0 [K]
       vbs_set(4)%Hvap_eff    = 1.2350e+05_dp/argas        ! eff. evap. enthalpy [K]
