...
!!$
!!$
!!$!    ! >> from mo_chemistry (not sure about this yet)
    ...
    DO jc = 1,nclass
       ...
       IF (sizeclass(jc)%lsoainclass) THEN
          ...
          DO jg = 1,vbs_ngroup
             ...
             js=zgroup%spid
             write(6,*) speclist(zgroup%spid)%shortname
       ...
       IF (sizeclass(jc)%lsoainclass) THEN
          ...
          DO jg = 1,vbs_ngroup
             ...
             pvols(jc,zgroup%id_vols) = zc_part_all(jvc,zgroup%id_vols)&
                  ...
                  *speclist(zgroup%spid)%moleweight*1e-3_dp
          END DO ! js
