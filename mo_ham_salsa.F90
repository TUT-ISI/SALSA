...
MODULE mo_ham_salsa
  ...
  !--> HK
  !REAL(dp), PUBLIC, ALLOCATABLE :: rwet_class(:,:,:)
  !<-- HK
...
CONTAINS
  ...
  SUBROUTINE salsa(&
    ...
    REAL(dp), INTENT(inout) ::         & ! gas phase concentrations at each grid point [#/m3]
         ...
         pm6dry(kbdim,klev,fn2b)   ,& ! dry radius [cm]
    ...
    DO kk = in1a,fn1a      ! size bin
       DO jj = 1,klev      ! vertical grid
          DO ii = 1,kproma ! horizontal grid
             ...
             zddry = (sum(zvols(ii,jj,kk,1:2))/pnaero(ii,jj,kk)/(pi/6.0))**(1._dp/3._dp)
    ...
    DO kk = in2a,fn2a      ! size bin
       DO jj = 1,klev      ! vertical grid
          DO ii = 1,kproma ! horizontal grid
             IF(pnaero(ii,jj,kk) < nlim) CYCLE
             zddry = (sum(zvols(ii,jj,kk,:))/pnaero(ii,jj,kk)/(pi/6.0))**(1._dp/3._dp)
             IF(zddry < 1.e-10_dp) THEN
                pc_gas(ii,jj,isubm_so4g) = pc_gas(ii,jj,isubm_so4g) + &
                     zvols(ii,jj,kk,1) * speclist(id_so4)%density /   &
                     (speclist(id_so4)%moleweight/1000.) * avo
    ...
    DO nn = 1, 2
       ...
       pm6dry(1:kproma,:,1:fn2b) = (zcore(1:kproma,:,1:fn2b)/(pi/6.0))**(1._dp/3._dp)/2._dp * 100._dp
       pm6rp(1:kproma,:,1:fn2b)  = zdwet(1:kproma,:,1:fn2b)/2._dp * 100._dp
       pww(1:kproma,:,1:fn2b)    = ((pi/6.0)*zdwet(1:kproma,:,1:fn2b)**3 -                            &
    ...
    ! << thk
    !-->HK
    IF (.NOT. ALLOCATED(rwet_class)) ALLOCATE(rwet_class(kproma,klev,nclass))
    IF (.NOT. ALLOCATED(rdry_class)) ALLOCATE(rdry_class(kproma,klev,nclass))
    IF (.NOT. ALLOCATED(densaer_class)) ALLOCATE(densaer_class(kproma,klev,nclass))
    IF (.NOT. ALLOCATED(aerwat_class)) ALLOCATE(aerwat_class(kproma,klev,nclass))


    rwet_class(1:kproma,:,1:fn2b) = pm6rp(1:kproma,:,1:fn2b)
    rdry_class(1:kproma,:,1:fn2b) = pm6dry(1:kproma,:,1:fn2b)
    densaer_class(1:kproma,:,1:fn2b) = pm6dry(1:kproma,:,1:fn2b)
    aerwat_class(1:kproma,:,1:fn2b) = prhop(1:kproma,:,1:fn2b)
    !<--HK
#endif
