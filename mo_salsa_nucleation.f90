!****************************************************************
!*                                                              *
!*   module MO_HAM_SALSA_NUCLEATION                                 *
!*                                                              *
...
CONTAINS
  ...
  SUBROUTINE nucleation(kproma, kbdim,  klev,   krow,                     &
                        pnaero, pdwet,                                    &
                        ptemp,  prh,    ppres,                            &
                        pcsa,   pcocnv, ptstep, pj3n3, pxsa, pxocnv, ppbl)
    ...
    REAL(dp), INTENT(IN) ::       &
         ...
         pdwet(kbdim,klev,fn2b),  & ! wet diameter of particles [m]
         ptstep,                  & ! timestep [s]
    ...
    REAL(dp), INTENT(INOUT) ::    &
         pnaero(kbdim,klev,fn2b), & ! number concentration in each bin [#/m3]
    ...
    CASE(4) ! ternary H2SO4-H2O-NH3 nucleation
       ...
       CALL finish('mo_salsa_nucleation','Specify NH3 mixing ratio')
...
!          IF (pcsa(ii,jj)+pcocnv(ii,jj) < 1._dp) CYCLE
          ...
          zknud = 2._dp*zmfp/(pdwet(ii,jj,:)+d_sa)     ! Knudsen number
          ...
          zcsink = sum(pdwet(ii,jj,:)*zbeta*pnaero(ii,jj,:))                    !(3)
          ...
          IF (nj3 == 1) THEN       ! Parametrization of J3 by Kerminen and Kulmala (2002)
             ...
             IF(zcsink < 1.e-30_dp) THEN 
                zeta = 0._dp
             ELSE
                zdmean = 1._dp/sum(pnaero(ii,jj,:))* &       ! mean diameter of backgroud population [nm]
                  sum(pnaero(ii,jj,:)*pdwet(ii,jj,:))*1.e9_dp
          ...
          ELSEIF (nj3 > 1) THEN
             ...
             zRc2 = zdcrit(ii,jj)/2._dp + pdwet(ii,jj,:)/2._dp ! [m]
             zRx2 = reglim(1)/2._dp + pdwet(ii,jj,:)/2._dp  ! [m]
             ...
             zm_2 = 4._dp/3._dp*pi*(pdwet(ii,jj,:)/2._dp)**3*rhosu!(rhosu*pxsa+rhooc*pxocnv)   ! [kg] 
             ...
             zknud_2 = 2._dp*zmfp/pdwet(ii,jj,:)
             ...
             zDc_2 = boltz*ptemp(ii,jj)*zCc_2/(3_dp*pi*zmyy*pdwet(ii,jj,:))          
             ...
             zCoagS_c= MAX(1.e-20_dp, sum(zK_c2*pnaero(ii,jj,:)))  ![1/s]       Coagulation sink for critical cluster           
             zCoagS_x= MAX(1.e-20_dp, sum(zK_x2*pnaero(ii,jj,:)))  !            Coagulation sink for 3 nm cluster      
             ...
             ELSE
                CALL finish('mo_salsa_nucleation','Error in chosen nj3')
             END IF
