...
CONTAINS
  ...
  SUBROUTINE vbs_gas_phase_chem(&
    ...
    TYPE(t_vbs_group), POINTER :: zradact ! "
    ...
    DO jv = 1,vbs_nvocs
       ...
       DO jk = 1,klev
          DO jl = 1,kproma
             ...
             ! Radioactive compounds for AEROCOM III
             IF(zprec%k_0_D > 0._dp) THEN

                !---amount of reacted voc
                zloss_prec = zc_prec*(1._dp - EXP(-zprec%k_0_D * time_step_len))
                ...
                zradact => vbs_set(1)
                idt_base = speclist(zradact%spid)%idt
                
                ! the production rate for jb
                zprodrate_vbs = zlossrate_prec * zprec%stoich_coeff(1)
    ...
    DO jc = 1,nclass
       ...
       IF (sizeclass(jc)%lsoainclass) THEN
          ...
          zc_part_all(jvc, subm_naerospec) =&
               (((pi/6.0)*pd_wet(jc)**3)*pnaero(jc)-zv_tot)*speclist(id_wat)%density/&
