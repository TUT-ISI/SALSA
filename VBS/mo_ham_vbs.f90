...
CONTAINS
  ...
  SUBROUTINE vbs_species
    ...
    USE mo_ham_species,        ONLY: &
         id_oc!, id_ocnv                ! for cross-referencing !eehol: leave id_ocnv out
    ...
    !eehol: commented whole case out as it is not used anymore   
    ...
    !    vbs_voc_prec(1)%k_0_D      = 0._dp      ! no decay
    ...
    !    vbs_voc_prec(2)%k_0_D      = 0._dp      ! no decay 
    ...
    !    vbs_voc_prec(3)%k_0_D      = 0.0_dp !no decay
    ...
    !    vbs_voc_prec(4)%k_0_D      = 0.0_dp !no decay
    ...
    !    vbs_voc_prec(5)%k_0_D      = 0.0_dp !no decay
    ...
    CASE(vbs_setup_3class) ! 3 class scheme with volatilities 0, 1, and 10 ug/m3
       ...
       vbs_voc_prec(1)%k_0_NO3    = 1.2E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(1)%Eact_p_NO3 = 490._dp !reduced activation energy [K]: Eact_p=Eact/R
       vbs_voc_prec(1)%k_0_D      = 0.0_dp !no decay
       ...
       vbs_voc_prec(2)%k_0_NO3    = 3.15E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(2)%Eact_p_NO3 = -450._dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       vbs_voc_prec(2)%k_0_D      = 0.0_dp !no decay
    ...
    CASE(vbs_setup_3class_allvoc) ! 3 class scheme with volatilities 0, 1, and 10 ug/m3 implementing all monoterpenes
      ...
      CALL new_species(&
       ...
       !oxidation rates and their temperature dependence (Arrhenius)
       ! OH (data from IUPAC)
       vbs_voc_prec(2)%k_0_OH     = 1.2E-11_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(2)%Eact_p_OH  = 440._dp    ! reduced activation energy [K]: Eact_p=Eact/R
       ! O3 (data from IUPAC)
       vbs_voc_prec(2)%k_0_O3     = 6.3E-16_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(2)%Eact_p_O3  = -580._dp   ! reduced activation energy [K]: Eact_p=Eact/R
       ! NO3
       vbs_voc_prec(2)%k_0_NO3    = 1.2E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(2)%Eact_p_NO3 = 490._dp !reduced activation energy [K]: Eact_p=Eact/R
       ...
       IF (laqsoa) THEN
          ! from Harri (hi NOx)
          !                            (  OC   ,  VBS1   ,  VBS10  , IEPOX , Glyx)
          vbs_voc_prec(2)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp, 0.0_dp, 0.0_dp/) 
       ELSE
          !                            (  OC      ,  VBS1   , VBS10    )  
          vbs_voc_prec(2)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp/) ! from Harri (hi NOx)
          !vbs_voc_prec(2)%stoich_coeff=(/0.002_dp, 0.003_dp, 0.065_dp/) ! from Harri (low NOx)
       ENDIF
      ...
      CALL new_species(&
       ...
       !oxidation rates and their temperature dependence (Arrhenius)
       ! OH (data from IUPAC)
       vbs_voc_prec(3)%k_0_OH     = 1.2E-11_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(3)%Eact_p_OH  = 440._dp    ! reduced activation energy [K]: Eact_p=Eact/R
       ! O3 (data from IUPAC)
       vbs_voc_prec(3)%k_0_O3     = 6.3E-16_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(3)%Eact_p_O3  = -580._dp   ! reduced activation energy [K]: Eact_p=Eact/R
       ! NO3
       vbs_voc_prec(3)%k_0_NO3    = 1.2E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(3)%Eact_p_NO3 = 490._dp !reduced activation energy [K]: Eact_p=Eact/R
       ...
       IF (laqsoa) THEN
          ! from Harri (hi NOx)
          !                            (  OC   ,  VBS1   ,  VBS10  , IEPOX , Glyx)
          vbs_voc_prec(3)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp, 0.0_dp, 0.0_dp/) 
       ELSE
          !                            (  OC      ,  VBS1   , VBS10    )  
          vbs_voc_prec(3)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp/) ! from Harri (hi NOx)
          !vbs_voc_prec(3)%stoich_coeff=(/0.002_dp, 0.003_dp, 0.065_dp/) ! from Harri (low NOx)
       ENDIF
      ...
      CALL new_species(&
       ...
       !oxidation rates and their temperature dependence (Arrhenius)
       ! OH (data from IUPAC)
       vbs_voc_prec(4)%k_0_OH     = 1.2E-11_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(4)%Eact_p_OH  = 440._dp    ! reduced activation energy [K]: Eact_p=Eact/R
       ! O3 (data from IUPAC)
       vbs_voc_prec(4)%k_0_O3     = 6.3E-16_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(4)%Eact_p_O3  = -580._dp   ! reduced activation energy [K]: Eact_p=Eact/R
       ! NO3
       vbs_voc_prec(4)%k_0_NO3    = 1.2E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(4)%Eact_p_NO3 = 490._dp !reduced activation energy [K]: Eact_p=Eact/R
       ...
       IF (laqsoa) THEN
          ! from Harri (hi NOx)
          !                            (  OC   ,  VBS1   ,  VBS10  , IEPOX , Glyx)
          vbs_voc_prec(4)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp, 0.0_dp, 0.0_dp/) 
       ELSE
          !                            (  OC      ,  VBS1   , VBS10    )  
          vbs_voc_prec(4)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp/) ! from Harri (hi NOx)
          !vbs_voc_prec(4)%stoich_coeff=(/0.002_dp, 0.003_dp, 0.065_dp/) ! from Harri (low NOx)
       ENDIF
      ...
      CALL new_species(&
       ...
       !oxidation rates and their temperature dependence (Arrhenius)
       ! OH (data from IUPAC)
       vbs_voc_prec(5)%k_0_OH     = 1.2E-11_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(5)%Eact_p_OH  = 440._dp    ! reduced activation energy [K]: Eact_p=Eact/R
       ! O3 (data from IUPAC)
       vbs_voc_prec(5)%k_0_O3     = 6.3E-16_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(5)%Eact_p_O3  = -580._dp   ! reduced activation energy [K]: Eact_p=Eact/R
       ! NO3
       vbs_voc_prec(5)%k_0_NO3    = 1.2E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(5)%Eact_p_NO3 = 490._dp !reduced activation energy [K]: Eact_p=Eact/R
       ...
       IF (laqsoa) THEN
          ! from Harri (hi NOx)
          !                            (  OC   ,  VBS1   ,  VBS10  , IEPOX , Glyx)
          vbs_voc_prec(5)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp, 0.0_dp, 0.0_dp/) 
       ELSE
          !                            (  OC      ,  VBS1   , VBS10    )  
          vbs_voc_prec(5)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp/) ! from Harri (hi NOx)
          !vbs_voc_prec(5)%stoich_coeff=(/0.002_dp, 0.003_dp, 0.065_dp/) ! from Harri (low NOx)
       ENDIF
      ...
      CALL new_species(&
             ...
             lwetdep     = .FALSE.,             &
             lburden     = .TRUE.,              &
             idx         = vbs_voc_prec(6)%spid &
             ) 

       !oxidation rates and their temperature dependence (Arrhenius)
       ! OH (data from IUPAC)
       vbs_voc_prec(6)%k_0_OH     = 1.2E-11_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(6)%Eact_p_OH  = 440._dp    ! reduced activation energy [K]: Eact_p=Eact/R
       ! O3 (data from IUPAC)
       vbs_voc_prec(6)%k_0_O3     = 6.3E-16_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(6)%Eact_p_O3  = -580._dp   ! reduced activation energy [K]: Eact_p=Eact/R
       ! NO3
       vbs_voc_prec(6)%k_0_NO3    = 1.2E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(6)%Eact_p_NO3 = 490._dp !reduced activation energy [K]: Eact_p=Eact/R

       vbs_voc_prec(6)%k_0_D      = 0.0_dp !no decay

       ! Stoichiometric Coefficients (different amount depending on laqsoa)
       IF (laqsoa) THEN
          ! from Harri (hi NOx)
          !                            (  OC   ,  VBS1   ,  VBS10  , IEPOX , Glyx)
          vbs_voc_prec(6)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp, 0.0_dp, 0.0_dp/) 
       ELSE
          !                            (  OC      ,  VBS1   , VBS10    )  
          vbs_voc_prec(6)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp/) ! from Harri (hi NOx)
          !vbs_voc_prec(6)%stoich_coeff=(/0.002_dp, 0.003_dp, 0.065_dp/) ! from Harri (low NOx)
       ENDIF
      ...
      CALL new_species(&
       ...
       !oxidation rates and their temperature dependence (Arrhenius)
       ! OH (data from IUPAC)
       vbs_voc_prec(7)%k_0_OH     = 1.2E-11_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(7)%Eact_p_OH  = 440._dp    ! reduced activation energy [K]: Eact_p=Eact/R
       ! O3 (data from IUPAC)
       vbs_voc_prec(7)%k_0_O3     = 6.3E-16_dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(7)%Eact_p_O3  = -580._dp   ! reduced activation energy [K]: Eact_p=Eact/R
       ! NO3
       vbs_voc_prec(7)%k_0_NO3    = 1.2E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(7)%Eact_p_NO3 = 490._dp !reduced activation energy [K]: Eact_p=Eact/R

       vbs_voc_prec(7)%k_0_D      = 0.0_dp !no decay

       ! Stoichiometric Coefficients (different amount depending on laqsoa)
       IF (laqsoa) THEN
          ! from Harri (hi NOx)
          !                            (  OC   ,  VBS1   ,  VBS10  , IEPOX , Glyx)
          vbs_voc_prec(7)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp, 0.0_dp, 0.0_dp/) 
       ELSE
          !                            (  OC      ,  VBS1   , VBS10    )  
          vbs_voc_prec(7)%stoich_coeff=(/0.1_dp, 0.037_dp, 0.088_dp/) ! from Harri (hi NOx)
          !vbs_voc_prec(7)%stoich_coeff=(/0.002_dp, 0.003_dp, 0.065_dp/) ! from Harri (low NOx)
       ENDIF
       ...
       CALL new_species(&
            ...
            lwetdep     = .FALSE.,                  &
            lburden     = .TRUE.,                   &
            idx         = vbs_voc_prec(8)%spid &
          ) 
       !oxidation rates and their temperature dependence (Arrhenius)
       !OH
       vbs_voc_prec(8)%k_0_OH     = 2.7E-11_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(8)%Eact_p_OH  = 390._dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !O3
       vbs_voc_prec(8)%k_0_O3     = 1.03E-14_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(8)%Eact_p_O3  = -1995._dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !NO3
       vbs_voc_prec(8)%k_0_NO3    = 3.15E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(8)%Eact_p_NO3 = -450._dp !reduced activation energy [K*mol]: Eact_p = Eact/R

       vbs_voc_prec(8)%k_0_D      = 0.0_dp !no decay

       ! Stoichiometric Coefficients
       IF (laqsoa) THEN
          !                            (  OC   ,  VBS1    ,   VBS10  ,   IEPOX , Glyx)
          vbs_voc_prec(8)%stoich_coeff=(/0.0_dp, 0.0295_dp, 0.0453_dp, 0.525_dp, 0.04_dp/) 
       ELSE
          !                            (  OC   ,  VBS1    , VBS10     )  
          vbs_voc_prec(8)%stoich_coeff=(/0.0_dp, 0.0295_dp, 0.0453_dp/) 
       ENDIF
       ...
       CALL new_species(&
            ...
            lwetdep     = .FALSE.,                  &
            lburden     = .TRUE.,                   &
            idx         = vbs_voc_prec(9)%spid &
            ) 
       !oxidation rates and their temperature dependence (Arrhenius)
       !OH
       vbs_voc_prec(9)%k_0_OH     = 1.81E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(9)%Eact_p_OH  = 338._dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !O3
       vbs_voc_prec(9)%k_0_O3     = 0.0_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(9)%Eact_p_O3  = 0.0_dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !NO3
       vbs_voc_prec(9)%k_0_NO3    = 0.0_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(9)%Eact_p_NO3 = 0.0_dp !reduced activation energy [K*mol]: Eact_p = Eact/R

       vbs_voc_prec(9)%k_0_D      = 0.0_dp !no decay

       ! Stoichiometric Coefficients
       IF (laqsoa) THEN
          !                            (  OC    ,  VBS1 , VBS10 , IEPOX , Glyx    )
          vbs_voc_prec(9)%stoich_coeff=(/0.36_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.24_dp/) ! to be reviewed
       ELSE
          !                            (  OC    ,  VBS1 ,   VBS10)
          vbs_voc_prec(9)%stoich_coeff=(/0.36_dp, 0.0_dp, 0.0_dp/) ! to be reviewed
       ENDIF
       ...
       CALL new_species(&
            ...
            lwetdep     = .FALSE.,                  &
            lburden     = .TRUE.,                   &
            idx         = vbs_voc_prec(10)%spid &
            ) 
       !oxidation rates and their temperature dependence (Arrhenius)
       !OH
       vbs_voc_prec(10)%k_0_OH     = 2.31E-11_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(10)%Eact_p_OH  = 0.0_dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !O3
       vbs_voc_prec(10)%k_0_O3     = 0.0_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(10)%Eact_p_O3  = 0.0_dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !NO3
       vbs_voc_prec(10)%k_0_NO3    = 2.6E-16_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(10)%Eact_p_NO3 = 0.0_dp !reduced activation energy [K*mol]: Eact_p = Eact/R

       vbs_voc_prec(10)%k_0_D      = 0.0_dp !no decay

       ! Stoichiometric Coefficients
       IF (laqsoa) THEN
          !                            (  OC    ,  VBS1 , VBS10 ,  IEPOX,  Glyx   )
          vbs_voc_prec(10)%stoich_coeff=(/0.30_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.25_dp/) ! to be reviewed
       ELSE
          !                            (  OC    ,  VBS1 , VBS10  )
          vbs_voc_prec(10)%stoich_coeff=(/0.30_dp, 0.0_dp, 0.0_dp/) ! to be reviewed
       ENDIF
       ...
       CALL new_species(&
            ...
            lwetdep     = .FALSE.,                  &
            lburden     = .TRUE.,                   &
            idx         = vbs_voc_prec(11)%spid &
            ) 
       !oxidation rates and their temperature dependence (Arrhenius)
       !OH
       vbs_voc_prec(11)%k_0_OH     = 2.33E-12_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(11)%Eact_p_OH  = -193._dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !O3
       vbs_voc_prec(11)%k_0_O3     = 0.0_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(11)%Eact_p_O3  = 0.0_dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !NO3
       vbs_voc_prec(11)%k_0_NO3    = 0.0_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(11)%Eact_p_NO3 = 0.0_dp !reduced activation energy [K*mol]: Eact_p = Eact/R

       vbs_voc_prec(11)%k_0_D      = 0.0_dp !no decay

       ! Stoichiometric Coefficients
       IF (laqsoa) THEN
          !                            (  OC    ,  VBS1 , VBS10 , IEPOX ,  Glyx   )
          vbs_voc_prec(11)%stoich_coeff=(/0.37_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.35_dp/) ! to be reviewed
       ELSE
          !                            (  OC    ,  VBS1 , VBS10  )
          vbs_voc_prec(11)%stoich_coeff=(/0.37_dp, 0.0_dp, 0.0_dp/) ! to be reviewed
       ENDIF
    ...
    CASE(vbs_setup_1class) ! 1 class scheme 222Rn in gas producing 210Pb with volatility of 0

       ! Defining the volatile species CO and 222Rn

       vbs_nvocs = 2

       ! One volatility class
       vbs_ngroup = 1
       ...
       !oxidation rates and their temperature dependence (Arrhenius)
       ! OH (no reaction)
       vbs_voc_prec(1)%k_0_OH     = 0._dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(1)%Eact_p_OH  = 440._dp    ! reduced activation energy [K]: Eact_p=Eact/R
       ! O3 (no reaction)
       vbs_voc_prec(1)%k_0_O3     = 0._dp ! pre-factor [m3/(mol*s)]
       vbs_voc_prec(1)%Eact_p_O3  = -580._dp   ! reduced activation energy [K]: Eact_p=Eact/R
       ! NO3
       vbs_voc_prec(1)%k_0_NO3    = 0._dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(1)%Eact_p_NO3 = 490._dp !reduced activation energy [K]: Eact_p=Eact/R
       ! Radioactive decay
       vbs_voc_prec(1)%k_0_D      = 2.11e-6 ! s-1

       ! Stoichiometric Coefficients (different amount depending on laqsoa)
       !                             Pb210 
       vbs_voc_prec(1)%stoich_coeff= (/1.0_dp/)
       ...
       !oxidation rates and their temperature dependence (Arrhenius)
       !OH
       vbs_voc_prec(2)%k_0_OH     = 0.0_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(2)%Eact_p_OH  = 390._dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !O3
       vbs_voc_prec(2)%k_0_O3     = 0.0_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(2)%Eact_p_O3  = -1995._dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       !NO3
       vbs_voc_prec(2)%k_0_NO3    = 0.0_dp !pre-factor [m3/(mol*s)]
       vbs_voc_prec(2)%Eact_p_NO3 = -450._dp !reduced activation energy [K*mol]: Eact_p = Eact/R
       ! Decay
       vbs_voc_prec(2)%k_0_D      = 2.315E-7_dp ! s-1
       ! CO only decays
       vbs_voc_prec(2)%stoich_coeff=(/0.0_dp/)

       ! VBS group 0 (C*= 0 mug/m3)
       CALL new_species(&
            nphase      = GAS_OR_AEROSOL,               &
            longname    = 'Lead 210',                   &
            shortname   = 'Pb210'   ,                   &
