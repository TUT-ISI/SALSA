...
MODULE mo_ham_vbsctl
  ...
  ! 3 VBS class setup with separate non-volatile class
  INTEGER, PUBLIC, PARAMETER :: vbs_setup_3class_allvoc = 2 

  ! 4 VBS class setup for AEROCOM III
  INTEGER, PUBLIC, PARAMETER :: vbs_setup_1class = 3
  ...
  TYPE, PUBLIC :: t_voc_prec
     ...
     ! Decay
     REAL(dp) :: k_0_D      ! pre-factor [mol/(m3*s)]
...
CONTAINS
  ...
  SUBROUTINE sethamvbs_log(nvbs_setup, laqsoa)
    ...
    CASE (vbs_setup_3class_allvoc)
       CALL message(&
            '',&
            'three VBS classes selected with all monoterpenes separately',  &
            level=em_param&
            )
    CASE (vbs_setup_1class)
       CALL message(&
            '',&
            'CO, Rn222 and Pb210 in VBS classes',  &
            level=em_param&
            )
    CASE DEFAULT
