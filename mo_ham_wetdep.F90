...
!#endif
  ...
  SUBROUTINE ham_wetdep(kproma, kbdim, klev, krow, ktop, kt,                          & 
                        ...
                        pdepintic_nucc, pdepintic_impw, pdepintic_impm,               &
                        pdepintic_impc, pnacttot)
  ...
  IF(kscavICtype > 0) THEN
     ...
     IF (IAND(kscavICphase,1) /= 0) THEN !water scavenging on (kscavICphase==1 .or. 3)
        ...
        CALL ic_scav(kproma, kbdim, klev, krow, ktop, kt, &
                     ...
                     reffi, reffl,   &
                     pnact, pfracn,                                  &
                     zdxtwat_nuc, zdxtwat_imp, zdxtwat, zxtwat, pnacttot)
     ...
     IF (IAND(kscavICphase,2) /= 0) THEN !ice scavenging on (kscavICphase==2 .or. 3)
        ...
        CALL ic_scav(kproma, kbdim, klev, krow, ktop, kt, &
                     ...
                     reffi, reffl,   &
                     pnact, pfracn,                                  &
                     zdxtice_nuc, zdxtice_imp, zdxtice, zxtice, pnacttot)
...
!        and tracer phase.
  ...
  SUBROUTINE ic_scav(kproma, kbdim, klev, krow, ktop, kt,           & !in
                     ...
                     reffi, reffl,                                  & !in
                     pnact, pfracn,                                  &
                     pdxt_nuc, pdxt_imp, pdxt, pxt, pnacttot)                   !out
  ...
  SUBROUTINE get_icscavfrac(kproma, kbdim, klev, krow, ktop, kt, kmod, & ! in
                            ...
                            reffi, reffl,                              & ! in
                            pnact, pfracn,                                  &
                            pfrac, pfrac_nuc, pfrac_imp, pnacttot)                 ! out
...
! Utility routine to compute the in-cloud scavenging fractions
                ...
                IF (lstrat) THEN !stratiform case SALSA
                   ...
                   SELECT CASE(kwat_phase)
                      CASE(1) !liq water
                         ...
                         pfrac_nuc(1:kproma,:) = pfracn(1:kproma,:,kmod) !pfrac_salsa comes from mo_ham_salsa_cloud.f90 (pfracn)
                      ...
                      CASE(2) !ice
                         ...
                         DO jj=1,nclass !calculate the total area of aerosols (this could be done outside of module)
                            ...
                            area_tot(1:kproma,:) = area_tot(1:kproma,:) + (4._dp*pi*(pm6rp(1:kproma,:,jj))**(2._dp)) &
                         ...
                         pfrac_nuc(1:kproma,:) = ((4._dp*pi*(pm6rp(1:kproma,:,kmod))**(2._dp)) &
  ...
  SUBROUTINE ic_scav_nuc(kproma, kbdim, klev, krow, kwat_phase, &
                         ktrac_phase, kmod, pm6rp,  pm6dry, pnact, pfracn,prhop1, pxtp1c_sav, pnacttot)
