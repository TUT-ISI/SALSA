...
MODULE mo_ham_salsactl
  ...
  PUBLIC :: nsnucl
  PUBLIC :: locgas, lsol2b, lhydr2b, act_coeff, nj3 !eehol: hydration flag for b-bins added
  ...
  !--- Physical:
  INTEGER :: nsnucl = 0 !2 ! Choice of the sulfate aerosol nucleation scheme:
                        !  nsnucl = 1  Binary
  ...
  LOGICAL :: locgas = .FALSE.,&   ! emission of organic carbon in gas phase
             lsol2b = .FALSE.,&   ! repartitioning of insoluble material in case of increase in solubility
         !    lhydr2b = .FALSE.    ! eehol: hydration calculation for insoluble material
             lhydr2b = .TRUE.    ! mirfan: turning on hydration in 2b
...
CONTAINS
  ...
  SUBROUTINE setham_salsa
    ...
    CALL p_bcast (lhydr2b, p_io) !eehol: hydration flag for b-bins added
    CALL p_bcast (locgas, p_io)
    CALL p_bcast (nj3,  p_io)

    !--- write the values of the switches:
    CALL sethamsalsa_log(locgas,lsol2b,lhydr2b,nsnucl) !eehol: hydration flag for b-bins added
#endif

  END SUBROUTINE setham_salsa

  SUBROUTINE sethamsalsa_log(locgas,lsol2b,lhydr2b,nsnucl) !eehol: hydration flag for b-bins added
    ...
    LOGICAL :: locgas,lsol2b,lhydr2b !eehol: hydration flag for b-bins added
    INTEGER :: nsnucl
    ...
    CALL print_value('lhydr2b', lhydr2b) !eehol: hydration flag for b-bins added
