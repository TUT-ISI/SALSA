...
CONTAINS
  ...
  SUBROUTINE set_sizebins()
    ...
    DO ii = in1a,fn1a
       vlolim(ii) = pi/6.0*(reglim(1)*ratio**(real(ii-1)/nbin(1)))**3
       vhilim(ii) = pi/6.0*(reglim(1)*ratio**(real(ii)/nbin(1)))**3
       dpmid(ii) = ((vhilim(ii) + vlolim(ii))/(2.*(pi/6.0)))**(1._dp/3._dp)
       vratiohi(ii)= (vhilim(ii))/((pi/6.0)*dpmid(ii)**3)
       vratiolo(ii)= (vlolim(ii))/((pi/6.0)*dpmid(ii)**3)
    ...
    DO jj = in2a,in2a+(nbin2-1)
       ...
       vlolim(jj) = (pi/6.0)*(reglim(2)*ratio**(real(ii)/real(nbin(2)-nbin3)))**3
       vhilim(jj) = (pi/6.0)*(reglim(2)*ratio**(real(ii+1)/real(nbin(2)-nbin3)))**3
       dpmid(jj) = ((vhilim(jj) + vlolim(jj))/(2._dp*(pi/6.0)))**(1._dp/3._dp)
       vratiohi(jj)= (vhilim(jj))/((pi/6.0)*dpmid(jj)**3)
       vratiolo(jj)= (vlolim(jj))/((pi/6.0)*dpmid(jj)**3)
    ...
    DO jj = fn2a-(nbin3-1),fn2a
       ...
       vlolim(jj) = (pi/6.0)*(reglim(3)*ratio**(real(ii)/real(nbin(2)-nbin2)))**3
       vhilim(jj) = (pi/6.0)*(reglim(3)*ratio**(real(ii+1)/real(nbin(2)-nbin2)))**3
       dpmid(jj) = ((vhilim(jj) + vlolim(jj))/(2._dp*(pi/6.0)))**(1._dp/3._dp)
       vratiohi(jj)= (vhilim(jj))/((pi/6.0)*dpmid(jj)**3)
       vratiolo(jj)= (vlolim(jj))/((pi/6.0)*dpmid(jj)**3)
  ...
  SUBROUTINE set_coagc(klev,pres0,temp0)
    ...
    mpart = (pi/6.0)*dpmid(1:fn2b)**3*1500.
  ...
  SUBROUTINE salsa_initialize
    ...
    DO jclass = in2b, fn2b
       ...
       sizeclass(jclass)%self       = 1
       !<--eehol: added here hydration flag for insoluble material to be treated as soluble
       IF (lhydr2b) THEN
          sizeclass(jclass)%lsoluble   = .TRUE.
       ELSE
       ...
       !-->eehol
       sizeclass(jclass)%lsed       = .TRUE.
       sizeclass(jclass)%lsoainclass =.TRUE.      ! .FALSE. !mirfan17Mar
       sizeclass(jclass)%lactivation = .TRUE. !>>dod<<
